...
>>> from CogVideoX_Muti_Prompt_pipline import CogVideoX_MultiPrompt_Pipeline
>>> import torch
...
from diffusers.utils import logging, replace_example_docstring, export_to_video
...
from Mutiple_prompt_mutiple_scene.utils import load_feature_extractor_from_ckpt, load_transformer_multi_ti2v, free_memory
logger = logging.get_logger(__name__)  # pylint: disable=invalid-name


mixed_precision_dict = {
    'fp16': torch.float16,
    'fp32': torch.float32,
    'bf16': torch.bfloat16,
}
...
def load_models(model_id):
    load_process = tqdm(range(7), desc='load pipline')

    tokenizer = AutoTokenizer.from_pretrained(model_id, subfolder='tokenizer', local_files_only=True)
    print('tokenizer加载完成')
    load_process.update(1)
    text_encoder = T5EncoderModel.from_pretrained(model_id, subfolder='text_encoder', local_files_only=True)
    print('text encoder加载完成')
    load_process.update(1)
    video_encoder = CLIPVisionModelWithProjection.from_pretrained(model_id, subfolder='Clip4Clip', local_files_only=True)
    print('video encoder加载完成')
    load_process.update(1)
    feature_extractor = load_feature_extractor_from_ckpt(os.path.join(model_id, 'feature_extractor', 'config.yaml'),
                                                         os.path.join(model_id, 'feature_extractor', 'feature_extraction_model_last.pth'))
    print('feature extractor 加载完成')
    load_process.update(1)
    transformer = load_transformer_multi_ti2v(model_id, subfolder='transformer', local_files_only=True)
    print('transformer加载完成')
    load_process.update(1)
    vae = AutoencoderKLCogVideoX.from_pretrained(model_id, subfolder='vae', local_files_only=True)
    print('vae 加载完成')
    load_process.update(1)
    scheduler = CogVideoXDDIMScheduler.from_pretrained(model_id, subfolder='scheduler', local_files_only=True)
    print('scheduler 加载完成')
    load_process.update(1)
    return tokenizer, text_encoder, video_encoder, feature_extractor, transformer, vae, scheduler


class CogVideoX_MultiPrompt_Pipeline:
    """
       ...
       """
    @classmethod
    def from_pretrained_local(cls, model_id, device=None, dtype=None):
        """加载模型"""
        tokenizer, text_encoder, video_encoder, feature_extractor, transformer, vae, scheduler = load_models(model_id)
        pipe = cls(
            tokenizer=tokenizer,
            text_encoder=text_encoder,
            image_encoder=video_encoder,
    ...
    def __init__(
        ...
        self.device = device if device is not None and (device == 'cpu' or 'cuda' in device) else ('cuda' if torch.cuda.is_available() else 'cpu')
        self.dtype = mixed_precision_dict[dtype] if dtype is not None else torch.float16
    ...
    def _get_t5_prompt_embeds(
        ...
        self.__move_to_device(self.text_encoder)
        ...
        self.__move_to_cpu(self.text_encoder)
    ...
    def _get_image_embeds(self, images: Union[List[str], str] = None,):
        """
        ...
        self.__move_to_device(self.image_encoder)
        image_features = encode_video(images, self.image_encoder, num_frames=49, device=self.device, dtype=self.dtype)
        self.__move_to_cpu(self.image_encoder)
    ...
    def _get_image_feature(self,
        ...
        if past_prompt is not None and len(past_prompt) != 0:
            past_prompt_embeds = self._get_t5_prompt_embeds(past_prompt)
        if past_image is not None and len(past_image) != 0:
            past_image_embeds = self._get_image_embeds(past_image)
        current_prompt_embeds = self._get_t5_prompt_embeds([current_prompt])
        self.__move_to_device(self.feature_extractor)
        current_image_feature = self.feature_extractor([past_image_embeds], [past_prompt_embeds], current_prompt_embeds)
        self.__move_to_cpu(self.feature_extractor)
        # 释放显存
        del current_prompt_embeds, past_prompt_embeds, past_image_embeds
        free_memory()

        current_image_feature = current_image_feature[0].to(dtype=self.dtype, device=self.device)
    ...
    def get_image_features(self,
        ...
        if current_image_features is not None:
            current_image_features = current_image_features.to(dtype=self.dtype, device=self.device)
        else:
            ...
            for i in range(len(current_prompts)):
                ...
                current_image_features.append(
                    self._get_image_feature(current_prompts[i], past_prompt, past_image)
                )

            current_image_features = torch.stack(current_image_features, dim=0).to(dtype=self.dtype, device=self.device)
        ...
        if do_image_feature_classifier_free_guidance and current_image_features_without_past == []:
            for i in range(len(current_prompts)):
                current_image_features_without_past.append(
                    self._get_image_feature(current_prompts[i], [], []))
        ...
        if isinstance(current_image_features_without_past, list) and len(current_image_features_without_past) != 0:
            ...
            current_image_features_without_past = current_image_features_without_past.to(device=self.device, dtype=self.dtype)

        return current_image_features, current_image_features_without_past

    def prepare_latents(
            self, batch_size, num_channels_latents, num_frames, height, width, generator, device, dtype, latents=None
    ):
        """
        Args:
    ...
    def decode_latents(self, latents: torch.Tensor) -> torch.Tensor:
        ...
        self.__move_to_device(self.vae)
        latents = latents.to(device=self.device, dtype=self.dtype)
        frames = self.vae.decode(latents).sample
        self.__move_to_cpu(self.vae)
        return frames
    ...
    def __call__(
        ...
        current_image_features, current_image_features_without_past = self.get_image_features(
            ...
            do_image_classifier_free_guidance, current_image_features_without_past)
        # 释放显存
        free_memory()
        ...
        self.__move_to_device(self.transformer)

        progress_bar = tqdm(total=num_inference_steps, desc='inference timestep')
        # for DPM-solver++
    ...
    def generate_video(
        ...
        video_frames = self.__call__(
                                        ...
                                        current_image_features_without_past=current_image_features_without_past,
                                        output_type='pil',
                                        return_dict=True,
                                        max_sequence_length=max_sequence_length
        ).frames[0]

        export_to_video(video_frames, output_video_path=save_video_path, fps=fps)
        print(f'保存视频到: {save_video_path}')
        return video_frames
    ...
    def __move_to_cpu(self, composes):
        """将模型加载到cpu"""
        composes = [composes] if not isinstance(composes, list) else composes
        for compose in composes:
            if hasattr(compose, 'to'):
                compose.to('cpu')

    def __move_to_device(self, composes):
        composes = [composes] if not isinstance(composes, list) else composes
        for compose in composes:
            if hasattr(compose, 'to'):
                compose.to(device=self.device, dtype=self.dtype)

    def move_all_compose_to_cpu(self):
        self.__move_to_cpu([self.text_encoder, self.image_encoder, self.feature_extractor, self.transformer])
