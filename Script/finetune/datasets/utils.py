...
def preprocess_video_with_buckets(
    ...
    resolution_buckets: List[Tuple[int, int, int]],     # 分辨率
) -> torch.Tensor:
    """
    Args:
        video_path: Path to the video file.
    ...
    The function processes the video through these steps:
        1. 找到最接近video帧数的预定帧，nearest_frame_bucket<=f
        2. 根据nearest_frame_bucket读取视频
        3. 找到最接近video分辨率的预定分辨率,[h,w]
        4. Resize video
    """
    video_reader = decord.VideoReader(uri=video_path.as_posix())
    video_num_frames = len(video_reader)
    # 取比video帧小的预定分辨率 [(f,h,w)]
    ...
    # 从video中均匀选择nearest_frame_bucket帧
    frame_indices = list(range(0, video_num_frames, video_num_frames // nearest_frame_bucket))
    frames = video_reader.get_batch(frame_indices)
    frames = frames[:nearest_frame_bucket].float()
    # 读取视频: [nearest_frame_bucket,c,h,w]
    frames = frames.permute(0, 3, 1, 2).contiguous()
