"""
copy from https://github.com/THUDM/CogVideo/tree/main/finetune
"""
...
from Mutiple_prompt_mutiple_scene.Script.finetune.constants import LOG_LEVEL, LOG_NAME, local_files_only
from Mutiple_prompt_mutiple_scene.Script.finetune.datasets.t2v_dataset import MultiTI2VDataset

from Mutiple_prompt_mutiple_scene.Script.finetune.datasets.utils import (
    load_images,
    load_prompts,
    load_videos,
...
from Mutiple_prompt_mutiple_scene.Script.finetune.schemas import Args, Components, State
from Mutiple_prompt_mutiple_scene.Script.finetune.utils import (
...
class Trainer:
    ...
    def __init__(self, args: Args) -> None:
        self.args = args
        self.state = State(
            weight_dtype=self.__get_training_dtype(),       # 精度
            train_frames=self.args.train_resolution[0],     # f
            train_height=self.args.train_resolution[1],     # H
            train_width=self.args.train_resolution[2],      # W
        )
        # 模型组件: vae,tokenizer,text_encoder,transformers,scheduler
    ...
    def prepare_models(self) -> None:
        """vae的enable_slicing和enable_tiling设置"""
    ...
    def prepare_dataset(self) -> None:
        raise NotImplementedError('prepare_dataset is not implemented.')
    ...
    def prepare_optimizer(self) -> None:
        ...
        optimizer = get_optimizer(
            params_to_optimize=params_to_optimize,      # 训练参数
            optimizer_name=self.args.optimizer,         # AdamW
            learning_rate=self.args.learning_rate,      # lr
            beta1=self.args.beta1,
            beta2=self.args.beta2,
            beta3=self.args.beta3,
            epsilon=self.args.epsilon,
            weight_decay=self.args.weight_decay,       # 正则化
    ...
    def __prepare_saving_loading_hooks(self, transformer_lora_config):
        ...
        def load_model_hook(models, input_dir):
            if not self.accelerator.distributed_type == DistributedType.DEEPSPEED:
                ...
                ).__class__.from_pretrained(self.args.model_path, subfolder="transformer", local_files_only=local_files_only)
